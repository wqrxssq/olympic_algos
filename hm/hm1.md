...
- "Позитивная" часть - сумма всех неотрицательных чисел в таких перестановках = (сумма всех подмножеств неотрицательных чисел из m элементов) * $m!$, чтобы ее посчитать левую часть давайте поссмотрим во сколько подмножеств войдет элемент $x$ - получим $\dbinom{m - 1}{i - 1}$, тогда очевидна формула - $m! * sum_{pos} * \dbinom{m - 1}{i - 1}$
- "Отрицательная" часть - сумма всех отрицательных чисел в таких перестановках = $sum_{neg} * \dbinom{m}{i}$

Тогда итоговая формула:
$$E(\xi) = \frac{1}{n!} * \displaystyle\sum_{i = 1}^{n} m! * sum_{pos} * \dbinom{m - 1}{i - 1} + sum_{neg} * \dbinom{m}{i}$$
